...
%f_PassiveForce = f_PassiveForce*1.2;
...
TS=TSt-y(20)-y(21)-y(22)-y(23); ER=exp(-RLa*(L_in-La)^2); 
Yh=fYv_lv*Yv*(1-exp(-gama*fgama_lv*(L_lv-y(25)-hwr)^2)); if (L_lv-y(25))>hwr; Yh=0.1*Yh; end;
Yh_lv = Yh;
fa=f*ER*ffa_lv; ga=Za+Yh; gd=Yd*exp(-Yc*(L_in-Lc)); 
...
L_in_rv = L_inter_rv(t);

TS_rv=TSt-y(43)-y(44)-y(45)-y(46); ER=exp(-RLa*(L_in_rv-La)^2); 
Yh=fYv_rv*Yv*(1-exp(-gama*fgama_rv*(L_rv-y(48)-hwr)^2)); if (L_rv-y(48))>hwr; Yh=0.1*Yh; end;
Yh_rv = Yh;
fa=f*ER*ffa_rv; ga=Za+Yh; gd=Yd*exp(-Yc*(L_in_rv-Lc)); 
...
% Pla=7.5*(Fm_la*Lm_la/Lr_la)*((rout_la/rint_la)^2-1)+Gamav_la*(y(26)-Vo_la)^3;
% Pla_parall=7.5*(Fparall_la*Lm_la/Lr_la)*((rout_la/rint_la)^2-1)+Gamav_la*(y(26)-Vo_la)^3;
Pla=0;
Pla_parall=0;

%--------Left ventricular pressure   new HEART 
fGeom_lv=1.;
% Plv=fGeom_lv*5*(Vw_lv/Kv_lv/Lr_lv)*(Fm_lv/Lm_lv^2);
% Plv_parall=fGeom_lv*5*(Vw_lv/Kv_lv/Lr_lv)*(Fparall_lv/Lm_lv^2);
...
% disp(sprintf('Prv1, Prv =   %d %d %d',Prv1,Prv,(Prv1/Prv))); 

%--------Valves and volumes
% Qmt=alfaIN*(exp(betaINleft*(Pla-Plv))-1);
% Qvaor=alfaOUT*(exp(betaOUTleft*(Plv-Paor))-1);
% Qtc=alfaIN*(exp(betaINright*(Pvc-Prv))-1);
% Qpv=alfaOUT*(exp(betaOUTright*(Prv-Ppa))-1);
...
DurationOpening=10;
RelativeAddedRvalve=10;


dp_lv=Plv-Paor;
% if tVaoOpen==0 && dp_lv>0
%   tVaoOpen=t;  
% end
Rvaor_eff=Rvaor*1;
% %%% Modif R valve
% Rfinal=Rvaor;
% Rini=(1+RelativeAddedRvalve)*Rfinal;
% tini=tVaoOpen;
% tfinal=tini+DurationOpening;
% a=(-Rini+Rfinal)/(DurationOpening^2);
% b=-2*a*tini;
% c=Rini-a*tini^2-b*tini;
% if tVaoOpen~=0 && t>tini && t<tfinal
% %   Rvaor_eff=-(t*(Rini-Rfinal)+(Rfinal*tini-Rini*tfinal))/DurationOpening;
% %   Rvaor_eff=Rini;
% %     Rvaor_eff=a*t^2+b*t+c;
% end
% %%% end of Modif R valve
...
% b=-2*a*tini;
% c=Rini-a*tini^2-b*tini;
% if tVpvOpen~=0 && t>tini && t<tfinal
% %   Rpv_eff=-(t*(Rini-Rfinal)+(Rfinal*tini-Rini*tfinal))/DurationOpening;
% %   Rpv_eff=Rini;
% %     Rpv_eff=a*t^2+b*t+c;
% end
% %%% end of Modif R valve
...
% if (Inert_ao~=0);
% % dp=Plv-y(28);
% % ydot(29)=(((Qvaor>0)+(dp>0))>0)*(dp-   (log(1+Qvaor/alfaOUT))/betaOUT    )/Inert_ao; %--------------Faor
% ydot(29)=(y(28)-y(30)-y(29)*Ras)/Inert_ao; %--------------Faor
% Qsys=(y(30)-Pvc)/Rsys;
% ydot(30)=(y(29)-Qsys)/Cas;%------------Pas
% % ydot(36)=0; %((y(30)-y(36))/Rsys-(y(36)-Pvc)/Rvs)/Cvs;%-Pvs
% ydot(28)=(Qvaor-y(29))/Caor; %------------------------Paor 
% end
if (Inert_ao==0);
Qsys=(y(28)-Pvc)/Rsys;
ydot(30)=0;%------------Pas
...
%----------Myocyte only---------
%  ydot(26)=0; ydot(27)=0; ydot(28)=0; ydot(29)=0; y(30)=0; 
%  ydot(33)=0; ydot(34)=0; ydot(35)=0; ydot(36)=0;
%  Plv=0; Prv=0; Pla=0; Qmt=0; Qmt=0;
%-------------------------------
...
% elseif (nargin == 7) && strcmp(runType,'Params')
%        output=[...
%         Ras ...   % 
%         Rsys ...   % 
%         Rpul ...   % 
